...
export const ProductsTab: React.FC<ProductsTabProps> = ({ search }) => {
  ...
  const { data: categoriesResp } = useSWR<Category[]>("categories", () =>
    getAllCategories().then(
      (r) => (r.data as unknown as { data: Category[] }).data ?? r.data,
    ),
  );

  const products = productsResp?.data ?? [];
  const total =
    productsResp?.meta?.total ?? productsResp?.pagination?.total ?? 0;
  ...
  const columns: VTableColumn<Product>[] = [
    ...
    },
    {
      ...
      render: (r) => (
        <Typography
          sx={{
            fontSize: 13,
            color: r.stock < 10 ? ADMIN_COLOR.red : ADMIN_COLOR.green,
            fontWeight: 600,
          }}
        >
          {r.stock}
    ...
    },
    {
      ...
      width: 80,
      render: (r) => {
        if (!r.badge || r.badge === ProductBadge.NONE)
          return (
        ...
        <Box
          sx={{
            ...
            bgcolor: r.is_active
              ...
              : "rgba(255,77,77,0.12)",
            color: r.is_active ? ADMIN_COLOR.green : ADMIN_COLOR.red,
            fontSize: 11,
            fontWeight: 600,
            px: 1.25,
            py: 0.375,
      ...
      render: (r) => (
        <Box sx={{ display: "flex", gap: 0.75 }}>
          ...
          <VButton
            variant="ghost"
            size="small"
            onClick={() => handleManageImages(r)}
          >
            Images
          </VButton>
          <VButton variant="ghost" size="small" onClick={() => handleEdit(r)}>
            Edit
          </VButton>
          <VButton
            variant="danger"
  ...
  return (
    <Box sx={{ p: 3.5 }}>
      ...
      >
        ...
        <VButton variant="secondary" onClick={handleAdd}>
          + Add Product
        </VButton>
      </Box>
      ...
      <Box sx={{ display: "flex", gap: 1, mb: 2.5, flexWrap: "wrap" }}>
        <Chip
          ...
          sx={{
            bgcolor:
              categoryFilter === null ? ADMIN_COLOR.accent : ADMIN_COLOR.s2,
            color:
              categoryFilter === null
                ? ADMIN_COLOR.accentDark
                : ADMIN_COLOR.dim,
        ...
        {categories.map((cat) => (
          <Chip
            ...
            sx={{
              bgcolor:
                categoryFilter === cat.id ? ADMIN_COLOR.accent : ADMIN_COLOR.s2,
              color:
